...
export default function TableOrder({ orderItems, ordersData }) {
    ...
    return (
        ...
        {orderItems.map((orderItem, key) => (
            <div key={key} className="grid grid-cols-5 border-t border-stroke px-4 py-4.5 dark:border-strokedark sm:grid-cols-7 md:px-6 2xl:px-7.5">
                <div className="col-span-3 flex gap-3 items-center justify-center">
                    <Image 
                      src={`/uploads/${orderItem.product.images[0]}`}
                      width={60}
                      height={50}
                      alt="Product"
                    />
                ...
                <div className="col-span-2 flex items-center justify-end">
                    <p className="text-sm text-black dark:text-white">
                       Rp {(orderItem.quantity * orderItem.product.price).toLocaleString().replaceAll(",",".")}
                    </p>
                </div>

            </div>
        ))}
            <div className="flex justify-between py-6 md:px-6 2xl:px-7.5 border-t border-stroke text-lg font-semibold text-black dark:text-white">
                <p>Totals</p>
